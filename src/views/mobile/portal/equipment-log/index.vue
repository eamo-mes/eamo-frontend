...
defineOptions({ name: 'MobilePortalEquipmentLogIndex' });
...
function handleQrScanned(payload: { rawText: string; matchedEquipment?: QrEquipmentItem }) {
  ...
  router.push(`/portal/equipment-log/${equipId}?scan_time=${encodeURIComponent(scanTime)}`);
}
    ...
    <div class="bg-white dark:bg-zinc-900 border-b border-slate-200/70 dark:border-zinc-800 px-4 py-3">
      <div class="flex items-center gap-3">
        ...
        <h1 class="text-sm font-bold text-slate-800 dark:text-zinc-200 m-0 flex-1 truncate">
          {{ t('page.portal.equipmentLogTitle') || 'Ghi Nhận Thông Số Thiết Bị' }}
        </h1>
        <span class="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-blue-50 dark:bg-blue-950/50 text-blue-600 dark:text-blue-400 shrink-0">
          {{ t('page.portal.step1Of2') || 'Bước 1/2' }}
        </span>
      </div>
    </div>

    <!-- ─── CONTENT: CAMERA QR SCANNER ─── -->
    <div class="pt-4 px-4 flex flex-col items-center pb-6">
      <QrCameraScanner
        ref="qrScannerRef"
        :equipments="equipments"
        viewport-id="qr-equipment-log-viewport"
