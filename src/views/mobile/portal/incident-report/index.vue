...
async function handleSubmitReport() {
  if (!selectedEquipment.value) {
    message.error(t('page.portal.msgSelectEquipFirst') || 'Vui lòng chọn thiết bị trước khi ghi nhận!');
    return;
  }
  ...
  try {
    ...
    const payload = {
      ...
      notes: formState.value.notes ? `[Mức độ: ${formState.value.severity.toUpperCase()}] ${formState.value.notes}` : `[Mức độ: ${formState.value.severity.toUpperCase()}] Báo cáo sự cố`,
    };
    ...
    message.success(t('page.portal.msgReportSuccess') || 'Đã ghi nhận sự cố thiết bị thành công!');
    stopCamera();
    router.push('/portal');
  } catch (err: any) {
  ...
  <div class="min-h-[90vh] bg-slate-50 dark:bg-zinc-950/40 pb-28 select-none">
      ...
      <div class="flex items-center gap-2.5">
        ...
        <div>
          ...
          <p class="text-[11px] text-slate-400 dark:text-zinc-500 m-0">
            {{ step === 1 ? (t('page.portal.step1Sub') || 'Bước 1/2: Xác định máy bị lỗi') : (t('page.portal.step2Sub') || 'Bước 2/2: Nhập thông tin sự cố') }}
          </p>
        </div>
      </div>
      ...
      <div class="relative bg-zinc-900 border border-zinc-800 rounded-2xl p-4 overflow-hidden shadow-sm text-center">
        ...
        <div class="mt-3 flex gap-2 justify-center">
          ...
          >
            ...
            {{ t('page.portal.btnSimulateScanSuccess') || 'Giả lập Quét thành công' }}
          </Button>
        </div>
      </div>
      ...
      <div>
        <div class="flex items-center justify-between mb-2">
          <h2 class="text-xs font-bold text-slate-700 dark:text-zinc-300 uppercase tracking-wider m-0">
            {{ t('page.portal.orSelectEquipmentManually') || 'Hoặc chọn thiết bị từ danh sách' }}
          </h2>
          <span class="text-[11px] text-slate-400 dark:text-zinc-500 font-mono">
            {{ filteredEquipments.length }} {{ t('page.ops.equipmentLabel') || 'máy' }}
          </span>
        </div>
        ...
        <div class="mb-3">
          <Input.Search
            ...
            :placeholder="t('page.portal.searchEquipPlaceholder') || 'Tìm theo mã máy hoặc tên thiết bị...'"
            allow-clear
        ...
        <Spin :spinning="loading">
          <div v-if="filteredEquipments.length > 0" class="space-y-2.5 max-h-[45vh] overflow-y-auto pr-0.5">
            ...
            >
              <div class="flex items-center gap-3 min-w-0">
                ...
                <div class="min-w-0">
                  ...
                  <p class="text-[11px] font-mono text-slate-400 dark:text-zinc-500 mt-0.5 mb-0 truncate">
                    {{ t('page.portal.codeLabel') || 'Mã' }}: <span class="text-indigo-600 dark:text-indigo-400 font-semibold">{{ equip.code }}</span>
                  </p>
                </div>
              </div>
              ...
              <Button type="default" size="small" class="rounded-lg text-xs font-semibold shrink-0">
                {{ t('page.portal.btnSelectEquip') || 'Chọn' }}
              </Button>
            </div>
          </div>
          ...
          <div v-else class="py-12 bg-white dark:bg-zinc-900 border border-dashed border-slate-200 dark:border-zinc-800 rounded-xl flex flex-col items-center justify-center">
            <Empty :description="t('page.portal.noEquipmentsFound') || 'Không tìm thấy thiết bị phù hợp'" />
          </div>
        </Spin>
      </div>
    ...
    <div v-else-if="step === 2 && selectedEquipment" class="p-4 space-y-4">
      ...
      <div class="bg-indigo-50/70 dark:bg-indigo-950/40 border border-indigo-200/80 dark:border-indigo-800/60 rounded-2xl p-3.5 flex items-center justify-between">
        <div class="flex items-center gap-3 min-w-0">
          ...
          <div class="min-w-0">
            <span class="text-[10px] uppercase font-bold text-indigo-600 dark:text-indigo-400 tracking-wider">{{ t('page.portal.selectedEquipmentHeader') || 'THIẾT BỊ ĐÃ CHỌN' }}</span>
            ...
            <p class="text-[11px] font-mono text-slate-500 dark:text-zinc-400 m-0">
              {{ t('page.portal.codeLabel') || 'Mã' }}: {{ selectedEquipment.code }}
            </p>
          </div>
        </div>
        ...
        <Button
          ...
          @click="step = 1"
        >
          {{ t('page.portal.btnSwitchEquip') || 'Đổi máy' }}
        </Button>
      </div>
      ...
      <div class="bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-800 rounded-2xl p-4 space-y-4 shadow-3xs">
        ...
        <div>
          <label class="block text-xs font-bold text-slate-700 dark:text-zinc-300 mb-1.5">
            {{ t('page.portal.labelErrorType') || 'Loại sự cố / Mã lỗi' }} <span class="text-red-500">*</span>
          </label>
          
          <Select
            v-model:value="formState.equipment_error_id"
            :placeholder="t('page.portal.placeholderSelectError') || '-- Chọn danh mục lỗi --'"
            class="w-full rounded-xl"
            allow-clear
          >
        ...
        <div>
          <label class="block text-xs font-bold text-slate-700 dark:text-zinc-300 mb-1.5">
            {{ t('page.portal.occurredAt') || 'Thời điểm phát hiện sự cố' }}
          </label>
          <DatePicker
        ...
        <div>
          <label class="block text-xs font-bold text-slate-700 dark:text-zinc-300 mb-1.5">
            {{ t('page.portal.severity') || 'Mức độ khẩn cấp' }}
          </label>
          <div class="grid grid-cols-3 gap-2">
            ...
            >
              ...
              {{ t('page.portal.severityLow') || 'Bình thường' }}
            </button>
            ...
            >
              ...
              {{ t('page.portal.severityMedium') || 'Cần kiểm tra' }}
            </button>
            ...
            >
              ...
              {{ t('page.portal.severityHigh') || 'Khẩn cấp' }}
            </button>
          </div>
        </div>
        ...
        <div>
          <label class="block text-xs font-bold text-slate-700 dark:text-zinc-300 mb-1.5">
            {{ t('page.portal.descriptionLabel') || 'Mô tả chi tiết sự cố / Hiện tượng hỏng hóc' }}
          </label>
          <Input.TextArea
            v-model:value="formState.notes"
            :rows="3"
            :placeholder="t('page.portal.descriptionPlaceholder') || 'Mô tả hiện tượng hỏng hóc (Ví dụ: Động cơ kêu to, rò rỉ dầu thủy lực tại van số 2...)'"
            class="rounded-xl"
          />
        </div>
    ...
    <div v-if="step === 2" class="fixed bottom-0 left-0 right-0 z-30 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-xl border-t border-slate-200/60 dark:border-zinc-800/60 p-4">
      ...
      >
        ...
        {{ t('page.portal.btnSubmitIncidentLog') || 'Ghi Nhận Sự Cố Thiết Bị' }}
      </Button>
    </div>
